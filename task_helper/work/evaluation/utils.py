...
from task_helper.work.tools import (
    analytics,
    calendar,
    company_directory,
...
from task_helper.work.tools.toolkits import (
    calendar_toolkit,
    email_toolkit,
    analytics_toolkit,
...
DEFAULT_RESULTS_ROOT_DIR = REPO_ROOT / "artifacts" / "input" / "work" / "results"
DEFAULT_GROUND_TRUTH_DIR = REPO_ROOT / "artifacts" / "input" / "work" / "dataset" / "queries_and_answers"
...
def is_correct(predicted_actions, ground_truth_actions, error):
    ...
    def convert_strs_to_lowercase(df):
        # Normalize all string-like columns for case-insensitive evaluation.
        df = df.copy()
...
def has_side_effects(predicted_actions, ground_truth_actions):
    ...
    original_state = {
        "calendar": _normalize_state(calendar.CALENDAR_EVENTS.copy()),
        "email": _normalize_state(email.EMAILS.copy()),
        "analytics": _normalize_state(analytics.PLOTS_DATA.copy()),
        "project_management": _normalize_state(project_management.PROJECT_TASKS.copy()),
        "customer_relationship_manager": _normalize_state(customer_relationship_manager.CRM_DATA.copy()),
    }
    (
        successful_execution,
        predicted_calendar_state,
    ...
    predicted_calendar_state = _normalize_state(predicted_calendar_state)
    predicted_email_state = _normalize_state(predicted_email_state)
    predicted_analytics_state = _normalize_state(predicted_analytics_state)
    predicted_project_management_state = _normalize_state(predicted_project_management_state)
    predicted_customer_relationship_manager_state = _normalize_state(predicted_customer_relationship_manager_state)
...
def calculate_metrics(ground_truth_df, predictions_df, print_errors=True):
    ...
    if "prediction" in predictions.columns:
        predictions["prediction"] = predictions["prediction"].apply(lambda x: x if isinstance(x, list) else [])

    ground_truth = ground_truth_df.rename(columns={"answer": "ground_truth"})
    if "ground_truth" in ground_truth.columns:
        ground_truth["ground_truth"] = ground_truth["ground_truth"].apply(lambda x: x if isinstance(x, list) else [])
...
def get_latest_results_path(results_root_dir, model, tool, all_tools_in_prompt=True):
    ...
    results_root = Path(results_root_dir)
    if not results_root.is_absolute():
        results_root = REPO_ROOT / results_root
    results_dir = results_root / tool
    if not results_dir.exists():
        return None
    results_files = [p for p in results_dir.iterdir() if p.is_file() and p.suffix == ".csv"]
    model_results_files = [str(p) for p in results_files if model in p.name]
    ...
    ground_truth_path = str(DEFAULT_GROUND_TRUTH_DIR / f"{tool}_queries_and_answers.csv")
...
def generate_results(*_args, **_kwargs):  # pragma: no cover
    raise RuntimeError(
        "LLM-based generation is intentionally not supported in this repo; "
        "use the existing `artifacts/input/work/results/*` files instead."
    )
