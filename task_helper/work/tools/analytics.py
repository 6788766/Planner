...
ANALYTICS_DATA = pd.read_csv(DATABASE_DIR / "analytics_data.csv", dtype=str)
...
def engaged_users_count(time_min=None, time_max=None):
    ...
    """
    if time_min:
        data = ANALYTICS_DATA.loc[ANALYTICS_DATA["date_of_visit"] >= time_min].copy()
    else:
        data = ANALYTICS_DATA.copy()
    if time_max:
        data = data.loc[data["date_of_visit"] <= time_max].copy()
    engaged = data["user_engaged"].astype(bool)
    return engaged.groupby(data["date_of_visit"]).sum().astype(int).to_dict()
...
def traffic_source_count(time_min=None, time_max=None, traffic_source=None):
    ...
    if traffic_source:
        data.loc[:, "visits_from_source"] = (data["traffic_source"] == traffic_source).astype(int)
...
def get_average_session_duration(time_min=None, time_max=None):
    ...
    data.loc[:, "session_duration_seconds"] = data["session_duration_seconds"].astype(float)
    return (
