from task_helper.travel.utils.func import get_valid_name_city,extract_before_parenthesis,extract_numbers_from_filenames
...
def _safe_float(value, default=0.0):
    """Convert value to float, falling back to default for None/invalid inputs."""
    if value is None:
        return default
    try:
...
def _safe_positive_int(value, default=0):
    """Convert value to a non-negative integer with fallback default."""
...
def get_total_cost(question, tested_data):
    total_cost = 0
    days = min(_safe_positive_int(question.get('days'), len(tested_data)), len(tested_data))
    people_number = _safe_positive_int(question.get('people_number'), 1) or 1
    for i in range(days):
        unit = tested_data[i]
        # transporation 
        if unit['transportation'] and  unit['transportation'] != '-':
            ...
            if org_city == None or dest_city == None:
                ...
                if 'flight number' in value.lower():
                    ...
                    if len(res) > 0:
                        price = _safe_float(res['Price'].values[0])
                        total_cost += price * people_number
                
                elif 'self-driving' in value.lower() or 'taxi' in value.lower():
                    if 'self-driving' in value.lower():
                        # print(org_city,dest_city)
                        info = googleDistanceMatrix.run_for_evaluation(org_city,dest_city,'self-driving')
                        cost = _safe_float(info.get('cost'))
                        total_cost += cost * math.ceil(people_number * 1.0 / 5)
                    else:
                        info = googleDistanceMatrix.run_for_evaluation(org_city,dest_city,'taxi')
                        cost = _safe_float(info.get('cost'))
        ...
        if unit['breakfast'] and unit['breakfast'] != '-':
            ...
            if len(res) > 0:
                avg_cost = _safe_float(res['Average Cost'].values[0])
                total_cost += avg_cost * people_number
        ...
        if unit['accommodation'] and unit['accommodation'] != '-':
            ...
            if len(res) > 0:
                price = _safe_float(res['price'].values[0])
                occupancy = _safe_positive_int(res['maximum occupancy'].values[0], 1) or 1
                total_cost += price * math.ceil(people_number * 1.0 / occupancy)
    # print(total_cost)
    return total_cost
