...
def eval_score(set_type: str, file_path: str):
    ...
    # Overall (mixed) pass rates:
    # - Local Pass Rate: micro average across commonsense + hard constraint checks.
    # - Global Pass Rate: macro average across all constraints (equivalent to Final Pass Rate).
    denom_map = {
        'train': {'commonsense': 360, 'hard': 105},
        'validation': {'commonsense': 1440, 'hard': 420},
        'test': {'commonsense': 8000, 'hard': 2290},
    }
    denoms = denom_map.get(set_type, {})
    commonsense_denom = denoms.get('commonsense', 0) or 0
    hard_denom = denoms.get('hard', 0) or 0
    total_denom = commonsense_denom + hard_denom
    if total_denom > 0:
        combined_pass = constraint_dis_record['commonsense']['pass'] + constraint_dis_record['hard']['pass']
        result['Local Pass Rate'] = combined_pass / total_denom
    else:
        result['Local Pass Rate'] = 0.0
    result['Global Pass Rate'] = result.get('Final Pass Rate', 0.0)
