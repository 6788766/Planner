...
def _load_city_state_set() -> list[str]:
    city_state_path = travel_dataset_root() / "database" / "background" / "citySet_with_states.txt"
    return city_state_path.read_text(encoding="utf-8").splitlines()
...
city_state_set = _load_city_state_set()
...
def is_reasonable_visiting_city(question, tested_data):
    ...
    for i in range(min(question['days'],len(tested_data))):
        city_value = tested_data[i].get('current_city') if isinstance(tested_data[i], dict) else None
        ...
        if 'from' in city_value:
            ...
            if not city1 or not city2:
                return False, f"Failed to parse move city from current_city: {city_value}"
    ...
    if not city_list:
        return False, "No valid city sequence found."
...
def is_valid_transportation(question, tested_data):
    ...
    first_day = tested_data[0] if isinstance(tested_data[0], dict) else {}
    first_transport = first_day.get('transportation')
    if isinstance(first_transport, str) and first_transport and first_transport != '-':
        transportation_list = [transportation_match(first_transport)]
    else:
        return False, "The transportation in day 1 should not be empty."
