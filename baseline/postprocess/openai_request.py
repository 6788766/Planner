...
KEY_POOL = [_API_KEY] if _API_KEY else []  # your key pool
if _API_KEY:
...
def limited_execution_time(func,model,prompt,temp,max_tokens=2048, default=None,**kwargs):
    try:
        if 'gpt-3.5-turbo' in model or 'gpt-4' in model or 'gpt-5' in model:
            result = func(
                    model=model,
                    messages=prompt,
                    temperature=temp
...
def catch_openai_api_error(prompt_input: list):
    ...
    # OpenAI Python SDK v1+ error types.
    ...
    timeout_error = (getattr(openai, "APITimeoutError", None), TimeoutError)
    connection_error = getattr(openai, "APIConnectionError", None)

    def _rotate_key(label: str) -> None:
        global KEY_INDEX
        if not KEY_POOL:
            print(f"{label}: {exc}")
            return
        ...
        print(f"{label}, now change the key. Current key index is {KEY_INDEX}.")
    ...
    if rate_limit is not None and isinstance(exc, rate_limit):
        _rotate_key("RateLimitError")
        return
    if connection_error is not None and isinstance(exc, connection_error):
        print(f"APIConnectionError: {exc}")
        return
    if api_error is not None and isinstance(exc, api_error):
        _rotate_key("APIError")
        return
    if auth_error is not None and isinstance(exc, auth_error):
        _rotate_key("AuthenticationError")
        return
    if any(t is not None and isinstance(exc, t) for t in timeout_error if t is not None):
        _rotate_key("TimeoutError")
        return

    print(f"API error: {type(exc).__name__}: {exc}")


def _chat_completion_create(*, model_name: str, messages: list, temperature: float):
    api_key = os.environ.get("OPENAI_API_KEY") or getattr(openai, "api_key", None)
    if not api_key:
        raise RuntimeError("OPENAI_API_KEY is required for TravelPlanner postprocess parsing.")
    ...
    # Legacy fallback (older SDKs).
    openai.api_key = api_key
    if hasattr(openai, "ChatCompletion"):
        return openai.ChatCompletion.create(model=model_name, messages=messages, temperature=temperature)
...
def prompt_chatgpt(system_input, user_input, temperature,save_path,index,history=[], model_name='gpt-4-1106-preview'):
    ...
    while True:
        try:
            completion = _chat_completion_create(model_name=model_name, messages=history, temperature=temperature)
            break
        except:
            catch_openai_api_error(user_input)
            time.sleep(1)

    # Support both legacy dict responses and newer OpenAI client objects.
    try:
        ...
        assistant_output = completion.choices[0].message.content  # type: ignore[attr-defined]
        usage = getattr(completion, "usage", None)
        total_prompt_tokens = getattr(usage, "prompt_tokens", 0) if usage is not None else 0
        total_completion_tokens = getattr(usage, "completion_tokens", 0) if usage is not None else 0
    history.append({"role": "assistant", "content": assistant_output})
    with open(save_path,'a+',encoding='utf-8') as f:
        assistant_output = str(index)+"\t"+"\t".join(x for x in assistant_output.split("\n"))
        f.write(assistant_output+'\n')
    return assistant_output, history, calc_cost_w_prompt(total_prompt_tokens, model_name) + calc_cost_w_tokens(total_completion_tokens, model_name)
...
-----EXAMPLE END-----
"""
    query_data_list = load_travelplanner_dataset(set_type)
